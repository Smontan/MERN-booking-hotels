...
router.get("/search", async (req: Request, res: Response) => {
  try {
    ...
    // set page size and page number for our hotels search result
    const pageSize = 5;
    ...
    //  display hotel according to the page number
    const hotels = await Hotel.find(query)
      .sort(sortOptions)
      .skip(skip)
      .limit(pageSize);
    // count the total hotels
...
router.post(
  ...
  verifyToken,
  async (req: Request, res: Response) => {
    // know the total cost, know the hotel Id, know the userid
